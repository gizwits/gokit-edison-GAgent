...
int GAgent_Socket_sendto(int sockfd, u8 *data, int len, void *addr, int addr_size)
{
    return sendto(sockfd, data, len, 0, (const struct sockaddr*)addr, addr_size);
}
int GAgent_Socket_accept(int sockfd, void *addr, int *addr_size)
{
    return accept(sockfd, (struct sockaddr*)addr, addr_size);
}
int GAgent_Socket_recvfrom(int sockfd, u8 *buffer, int len, void *addr, int *addr_size)
{
...
void GAgent_Socket_CreateTCPServer(int tcp_port)
{
    int bufferSize;
    struct sockaddr_t addr;

    if (g_GAgent_TCPServerFd==-1)
    {
        g_GAgent_TCPServerFd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);

        if(g_GAgent_TCPServerFd < 0)
        {
            GAgent_Printf(GAGENT_DEBUG, "Create TCPServer failed.errno:%d", errno);
            g_GAgent_TCPServerFd = -1;
            return;
        }
        if(Gagent_setsocketnonblock(g_GAgent_TCPServerFd) != 0)
        {
            GAgent_Printf(GAGENT_DEBUG,"TCP Server Gagent_setsocketnonblock fail.errno:%d", errno);
        }

        bufferSize = SOCKET_TCPSOCKET_BUFFERSIZE;
        if(setsockopt(g_GAgent_TCPServerFd, SOL_SOCKET, SO_RCVBUF, &bufferSize, 4)<0)
        {
        ...
        bufferSize = SOCKET_TCPSOCKET_BUFFERSIZE;
        if(setsockopt(g_GAgent_TCPServerFd,SOL_SOCKET,SO_SNDBUF,&bufferSize,4) < 0)
        {
        ...
        if(bind(g_GAgent_TCPServerFd, (struct sockaddr *)&addr, sizeof(addr)) != 0)
        {
            GAgent_Printf(GAGENT_DEBUG, "TCPSrever socket bind error,errno:%d", errno);
            close(g_GAgent_TCPServerFd);
            g_GAgent_TCPServerFd = -1;
            return;
        }

        if(listen(g_GAgent_TCPServerFd, 0) != 0)
        {
    ...
    GAgent_Printf(GAGENT_DEBUG,"TCP Server socketid:%d on port:%d", g_GAgent_TCPServerFd, tcp_port);
    return;
}

void GAgent_Socket_CreateUDPServer(int udp_port)
{
    struct sockaddr_t addr;
    if (g_GAgent_UDPServerFd==-1)
    {

        g_GAgent_UDPServerFd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
        if(g_GAgent_UDPServerFd < 1)
        {
            GAgent_Printf(GAGENT_DEBUG, "UDPServer socket create error,errno:%d", errno);
            g_GAgent_UDPServerFd = -1;
            return ;
        }
        //Only for Linux
        if(Gagent_setsocketnonblock(g_GAgent_UDPServerFd) != 0)
        {
        ...
        addr.sin_addr.s_addr = INADDR_ANY;
        if(bind(g_GAgent_UDPServerFd, (struct sockaddr *)&addr, sizeof(addr)) != 0)
        {
            GAgent_Printf(GAGENT_DEBUG, "UDPServer socket bind error,errno:%d", errno);
            close(g_GAgent_UDPServerFd);
            g_GAgent_UDPServerFd = -1;
            return ;
        }

    }
    GAgent_Printf(GAGENT_DEBUG,"UDP Server socketid:%d on port:%d", g_GAgent_UDPServerFd, udp_port);
    return;
}

void GAgent_Socket_CreateUDPBroadCastServer( int udp_port )
{
    ...
    if( g_GAgent_UDPBroadcastServerFd == -1 )
    {
        ...
        g_GAgent_UDPBroadcastServerFd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);

        if(g_GAgent_UDPBroadcastServerFd < 0)
        {
            GAgent_Printf(GAGENT_DEBUG, "UDP BC socket create error,errno:%d", errno);
            g_GAgent_UDPBroadcastServerFd = -1;
            return ;
        }

        //Only for Linux
        if(Gagent_setsocketnonblock(g_GAgent_UDPBroadcastServerFd) != 0)
        {
            GAgent_Printf(GAGENT_DEBUG,"UDP BC Server Gagent_setsocketnonblock fail.errno:%d", errno);
        }
        g_GAgent_stUDPBroadcastAddr.sin_family = AF_INET;
        g_GAgent_stUDPBroadcastAddr.sin_port=htons(udp_port);
        g_GAgent_stUDPBroadcastAddr.sin_addr.s_addr=htonl(INADDR_BROADCAST);

        if(setsockopt(g_GAgent_UDPBroadcastServerFd, SOL_SOCKET, SO_BROADCAST, &udpbufsize,sizeof(int)) != 0)
        {
            GAgent_Printf(GAGENT_DEBUG,"UDP BC Server setsockopt error,errno:%d", errno);
            //return;
        }
        if(bind(g_GAgent_UDPBroadcastServerFd, (struct sockaddr *)&g_GAgent_stUDPBroadcastAddr, sizeof(g_GAgent_stUDPBroadcastAddr)) != 0)
        {
            GAgent_Printf(GAGENT_DEBUG,"UDP BC Server bind error,errno:%d", errno);
            close(g_GAgent_UDPBroadcastServerFd);
            g_GAgent_UDPBroadcastServerFd = -1;
            return;
        }

    }
    GAgent_Printf(GAGENT_DEBUG,"UDP BC Server socketid:%d on port:%d", g_GAgent_UDPBroadcastServerFd, udp_port);
    return;
}
