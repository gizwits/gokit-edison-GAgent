...
typedef struct SocketClent
{
    ...
    int TimeOut; 
} GAGENT_SOCKET_CLIENT_S;
static GAGENT_SOCKET_CLIENT_S g_GAgent_stTCPSocketClients[SOCKET_MAXTCPCLENT];

/***********          GLOBAL VARIABLES           ***********/
u8 *g_GAgent_stSocketRecBuffer=NULL;
int g_GAgent_TCPServerFd = -1;
int g_GAgent_UDPServerFd = -1;
int g_GAgent_UDPBroadcastServerFd=-1;
int g_GAgent_TCPSocketMaxClientFd=0;
struct sockaddr_t g_GAgent_stUDPBroadcastAddr;//用于UDP广播

void GAgent_Socket_ResetClientTimeout(int socketfd)
{
    int i;

    for (i = 0; i < SOCKET_MAXTCPCLENT; i++)
    {
        if (g_GAgent_stTCPSocketClients[i].SocketFD == socketfd)
        {
...
void Init_Service(void)
{
    ...
    memset((void *)&g_GAgent_stTCPSocketClients, 0x0, sizeof(g_GAgent_stTCPSocketClients));

    for (i = 0; i < SOCKET_MAXTCPCLENT; i++)
    {
        g_GAgent_stTCPSocketClients[i].SocketFD = -1;
        g_GAgent_stTCPSocketClients[i].TimeOut = 0;
    }

    g_GAgent_stSocketRecBuffer = (unsigned char *) malloc(SOCKET_RECBUFFER_LEN);
    if (g_GAgent_stSocketRecBuffer == NULL)
    {
        /**/
        ;
    }        
    GAgent_CreateTimer(GAGENT_TIMER_PERIOD, 1000, GAgent_Socket_ClientTimeoutTimer);
    return;    
}


int GAgent_Socket_Init(void)
{
    GAgent_Socket_CreateTCPServer(12416);
    GAgent_Socket_CreateUDPServer(12414);
    GAgent_Socket_CreateUDPBroadCastServer(2415);
    return 0;
}
...
*
***********************************************/
void GAgent_Build_BroadCastPacket(  int UdpFlag,u8* Udp_Broadcast )
{
    int i;    
...
*
*******************************************/
void GAgent_Socket_SendBroadCastPacket( int time )
{
    int i;
    int ret;
    u8 Udp_Broadcast[200];
    
    if (g_GAgent_UDPBroadcastServerFd == -1)
    {
        return;
    }
    
    GAgent_Build_BroadCastPacket( 1,Udp_Broadcast );   

    for( i=0;i<time;i++)
    {
        if(g_GAgent_UDPBroadcastServerFd>-1)
        {
            ret = GAgent_Socket_sendto(g_GAgent_UDPBroadcastServerFd, Udp_Broadcast,Udp_Broadcast[4]+5, &g_GAgent_stUDPBroadcastAddr, sizeof(g_GAgent_stUDPBroadcastAddr));            
...
*******************************************************************/
void GAgent_Socket_DoTCPServer(void)
{
    int i,j;
    fd_set readfds, exceptfds;
    int recdatalen;
    ...
    for (i = 0; i < SOCKET_MAXTCPCLENT; i++)
    {
        if (g_GAgent_stTCPSocketClients[i].SocketFD != -1)
        {
            FD_SET(g_GAgent_stTCPSocketClients[i].SocketFD, &readfds);
        }
    }
    /*因为是循环处理，所以这个地方是立即返回*/
    ret = select(g_GAgent_TCPSocketMaxClientFd+1, &readfds, NULL, &exceptfds, &t);
    if (ret <0)  //0 成功 -1 失败
    {
        /*没有TCP Client有数据*/
        return;
    ...
    for(i=0;i<SOCKET_MAXTCPCLENT;i++) 
    {    
        if ((g_GAgent_stTCPSocketClients[i].SocketFD != -1) && (FD_ISSET(g_GAgent_stTCPSocketClients[i].SocketFD, &readfds)))        
        {
            memset(g_GAgent_stSocketRecBuffer, 0x0, SOCKET_RECBUFFER_LEN);
            recdatalen = recv(g_GAgent_stTCPSocketClients[i].SocketFD, g_GAgent_stSocketRecBuffer, SOCKET_RECBUFFER_LEN, 0);
            if (recdatalen > 0)
            {
                if (recdatalen < SOCKET_RECBUFFER_LEN)
                {
                    /*一次已经获取所有的数据，处理报文*/
                    GAgent_Socket_ResetClientTimeout(g_GAgent_stTCPSocketClients[i].SocketFD);
                    DispatchTCPData(g_GAgent_stTCPSocketClients[i].SocketFD, g_GAgent_stSocketRecBuffer, recdatalen);                    
                }
                else
                {
...
*
****************************************************************/
int GAgent_Socket_SendData2Client(u8* pdata, int datalength,unsigned char Cmd )
{
    int j,i;   
    int ret;
    if (pdata == NULL)
    ...
    for(j=0;j<8;j++)
    {
        if(g_GAgent_stTCPSocketClients[j].SocketFD>0)
        {
            if(Cmd==0x91)
            {
                for( i=0;i<8;i++ )
                {
                    if((g_GAgent_stTCPSocketClients[j].SocketFD == g_SocketLogin[i]) )
                    {
                        GAgent_Printf(GAGENT_INFO,"send data(len:%d) to TCP client [%d]", 
                        datalength, g_GAgent_stTCPSocketClients[j].SocketFD);
                        ret = send(g_GAgent_stTCPSocketClients[j].SocketFD, pdata, datalength,0);
                    }
                }
            }
            else
